...
- **Bootstrap and install the repository:**
  ...
  - `uv pip install -e .` -- installs dpdata in development mode with core dependencies (numpy, scipy, h5py, PyYAML, wcmatch, lmdb, msgpack-numpy)
...
- Core: numpy>=1.14.3, scipy, h5py, PyYAML, wcmatch, lmdb, msgpack-numpy
...
- **Installation timeouts:** Network timeouts during `uv pip install` are common. If this occurs, try:
  ...
  - Individual package installation: `uv pip install numpy scipy h5py PyYAML wcmatch lmdb msgpack-numpy`
