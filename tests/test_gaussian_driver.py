...
@unittest.skipIf(
    importlib.util.find_spec("openbabel") is None, "openbabel is not installed"
)
class TestGaussianDriver(unittest.TestCase, CompSys, IsNoPBC):
    ...
    def setUpClass(cls):
        ...
        cls.system_2 = cls.system_1.predict(
            keywords="force B3LYP", charge=1, driver="gaussian"
        )
        cls.places = 6

    def test_energy(self):
        self.assertAlmostEqual(self.system_2["energies"].ravel()[0], 0.0)
...
class TestMakeGaussian(unittest.TestCase):
    ...
    def test_make_fp_gaussian_multiplicity_one(self):
        self.system.to_gaussian_gjf(
            "gaussian/tmp.gjf", keywords="wb97x/6-31g* force", multiplicity=1
        )
...
class TestDumpGaussianGjf(unittest.TestCase):
    ...
    def test_dump_to_gjf(self):
        self.system.to_gaussian_gjf(
            "gaussian/tmp.gjf", keywords="force B3LYP/6-31G(d)", multiplicity=1
        )
