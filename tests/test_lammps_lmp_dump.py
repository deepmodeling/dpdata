...
class TestLmpRotateTriAngle(unittest.TestCase):
    ...
    def test_simple_cubic(self):
        ...
        rotated_cell, rotated_coords = rotate_to_lower_triangle(cubic_cell, cubic_coords)

        assert np.allclose(rotated_cell, np.diag([5, 5, 5]))
        assert np.allclose(rotated_coords, cubic_coords)
    ...
    def test_triclinic_cell(self):
        ...
        triclinic_coords = np.array([
            [0.5, 0.5, 0.5], 
            [3.0, 2.0, 1.0]  
        ])
        rotated_cell, rotated_coords = rotate_to_lower_triangle(triclinic_cell.copy(), triclinic_coords.copy())

        assert np.isclose(rotated_cell[0, 1], 0, atol=1e-10)
        assert np.isclose(rotated_cell[0, 2], 0, atol=1e-10)
        assert np.isclose(rotated_cell[1, 2], 0, atol=1e-10)

        assert rotated_cell[0, 0] > 0
        assert rotated_cell[1, 1] > 0
        assert rotated_cell[2, 2] > 0

        # check the distance between two atoms
        assert np.isclose(np.linalg.norm(rotated_coords[0] - rotated_coords[1]), 
    ...
    def test_negative_diagonal(self):
        ...
        assert np.isclose(np.linalg.norm(negative_coords[0] - negative_coords[1]), 
