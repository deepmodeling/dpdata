...
class TestDeepmdLoadDumpComp(unittest.TestCase, CompLabeledSys, IsPBC):
    def setUp(self):
        ...
        self.system_1.to_deepmd_npy("tmp.deepmd.npy", prec=np.float64, set_size=2)

        self.system_2 = dpdata.LabeledSystem(
            "tmp.deepmd.npy", fmt="deepmd/npy", type_map=["O", "H"]
        )
        self.places = 6
        self.e_places = 6
        self.f_places = 6
...
class TestDeepmdCompNoLabels2(unittest.TestCase, CompSys, IsPBC):
    def setUp(self):
        ...
        self.system_1 = dpdata.System(
            self.dir_name, fmt="deepmd/npy", type_map=["O", "H"]
        )
        self.system_2 = dpdata.System()
