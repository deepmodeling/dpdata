...
@unittest.skipIf(
    skip_bond_order_system,
    "dpdata does not have BondOrderSystem. One may install rdkit to fix.",
)
class TestBondOrderSystem(unittest.TestCase):
    ...
    def test_from_mol_file(self):
        syst = dpdata.BondOrderSystem(
            "bond_order/CH3OH.mol", fmt="mol", type_map=["O", "C", "H"]
        )
    ...
    def test_from_sdf_file_err(self):
        self.assertRaises(
            ValueError, dpdata.BondOrderSystem, "bond_order/methane_ethane.sdf"
        )

    def test_regularize_formal_charges(self):
        non_regular = Chem.MolFromMolFile(
            "bond_order/formal_charge.mol", removeHs=False
        )
    ...
    def test_sanitize_mol_obabel(self):
        ...
        for sdf_file in glob.glob("bond_order/refined-set-ligands/obabel/*sdf"):
            syst = dpdata.BondOrderSystem(
                sdf_file, sanitize_level="high", verbose=False
            )
