...
class QECPTrajFormat(Format):
    ...
    def from_system(self, file_name, begin = 0, step = 1, **kwargs):
        data, _ = dpdata.qe.traj.to_system_data(file_name + '.in', file_name, begin = begin, step = step)
        data['coords'] \
            = dpdata.md.pbc.apply_pbc(data['coords'],
                                      data['cells'],
            )
        return data
    ...
    def from_labeled_system(self, file_name, begin = 0, step = 1, **kwargs):
        data, cs = dpdata.qe.traj.to_system_data(file_name + '.in', file_name, begin = begin, step = step)
        data['coords'] \
            = dpdata.md.pbc.apply_pbc(data['coords'],
                                      data['cells'],
            )
        data['energies'], data['forces'], es \
            = dpdata.qe.traj.to_system_label(file_name + '.in', file_name, begin = begin, step = step)
